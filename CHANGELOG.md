...
- **审批坞**：审批请求改为对话区底部的坞式面板集中展示与处理，支持折叠/展开，重构对话区布局与审批交互
- **审批档位无中断切换**：切换 always-ask / write / yolo 不再重建空闲会话进程，即时生效无延迟
- **会话状态指示**：项目栏 session 行显示运行中 / 完成状态图标，补充无障碍语义
- **Ask 多字段表单**：适配 omp v18 的多字段 Ask 表单，支持多选自定义回答与默认值
- **附件预览**：移入输入卡片，与输入区统一主题背景
- **工程化**：引入 ESLint（flat config）、Stylelint、Prettier 与 `tsc --noEmit` 类型检查，统一代码格式

### 修复

- 停止按钮提前消失 / 切换审批档位丢消息 / deferred 请求无提示（回合状态改为计数器跟踪）
- PLAN 模式下方案文档重复显示，执行计划收敛至右上角可折叠面板
- Ask 表单提交与默认值处理加固
- 微调对话区背景与工具卡片配色
