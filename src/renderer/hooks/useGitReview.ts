...
export function useGitReview(app: ReturnType<typeof useAppCore>) {
  const refreshDiff = useCallback(async (source: ReviewSource, project = app.selectedProjectRef.current) => {
    ...
    try {
      ...
      app.setDiffStatus(result.diff ? `已读取${sourceLabel}改动` : result.message || `当前没有${sourceLabel}改动`);
    } catch (error) {
      if (
        refreshId !== app.diffRefreshIdRef.current ||
        app.selectedProjectRef.current?.path !== project.path ||
  ...
  const refreshGitBranches = useCallback(async (project = app.selectedProjectRef.current) => {
    ...
    if (refreshId !== app.branchRefreshIdRef.current || app.selectedProjectRef.current?.path !== project.path) {
      return null;
    }
  ...
  const syncGitReview = useCallback(async (project = app.selectedProjectRef.current) => {
    if (!project) {
